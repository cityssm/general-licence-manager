...
(() => {
    ...
    const renderBatchTransactions = () => {
        ...
        for (const transaction of batchTransactions) {
            ...
            if (licenceTotalsMap.has(transaction.licenceId)) {
                licenceTotalsMap.set(transaction.licenceId, licenceTotalsMap.get(transaction.licenceId) +
            ...
            if (batchTotalsMap.has(transaction.batchDateString)) {
                batchTotalsMap.set(transaction.batchDateString, batchTotalsMap.get(transaction.batchDateString) +
                    transaction.transactionAmount);
            }
            else {
    ...
    const splitOutstandingBalance = (clickEvent) => {
        ...
        if (batchDateStrings.length === 0) {
            ...
            bulmaJS.confirm({
                ...
                message: 'Are you sure you want to evenly split the outstanding balance across ' +
    ...
    const splitOutstandingBalances = (clickEvent) => {
        ...
        if (batchDateStrings.length === 0) {
            ...
            bulmaJS.confirm({
                ...
                message: 'Are you sure you want to evenly split the outstanding balances across ' +
                    batchDateStrings.length +
                    ' batch' +
                    (batchDateStrings.length === 1 ? '' : 'es') +
                    '?',
    ...
    const clearLicenceTransactions = (clickEvent) => {
        clickEvent.preventDefault();
        const licenceId = clickEvent.currentTarget.closest('tr')
            .dataset.licenceId;
        const doClear = () => {
