...
(() => {
  ...
  const startDateStringMinElement = document.querySelector("#filter--startDateStringMin") as HTMLInputElement;
  const startDateStringMaxElement = document.querySelector("#filter--startDateStringMax") as HTMLInputElement;
  ...
  const resultsElement = document.querySelector("#container--licenceCategorySummary") as HTMLElement;
  ...
  const getLicenceCategorySummary = () => {
    ...
    cityssm.postJSON(urlPrefix + "/licences/doGetLicenceCategorySummary",
      filterFormElement,
      (responseJSON: { licences: recordTypes.Licence[] }) => {
        ...
        try {
          ...
          const licenceTableBodyElements =
            new Map<string, HTMLTableSectionElement>();

          const summaryData =
            new Map<string, { licenceCategoryKey: string; licenceCategory: string; licenceCount: number; transactionAmountSum: number; }>();
          ...
          for (const licence of responseJSON.licences) {
            ...
            if (!licenceTableBodyElements.has(licence.licenceCategoryKey)) {
              ...
              licenceTableBodyElements.set(licence.licenceCategoryKey,
          ...
          summaryTableElement.querySelector("tfoot tr").insertAdjacentHTML("beforeend",
            "<td class=\"has-text-right\">" + responseJSON.licences.length + "</td>" +
