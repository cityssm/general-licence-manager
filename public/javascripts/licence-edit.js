...
(() => {
    ...
    if (!isCreate) {
        ...
        if (addAdditionalFeeButtonElement) {
            optionalAdditionalFees =
                licenceCategory.licenceCategoryAdditionalFees.filter((additionalFee) => {
    ...
    const getOutstandingBalance = () => {
        ...
        const transactionAmountTotalString = licenceTransactionsTableElement
            ...
            .textContent.slice(1);
        const outstandingBalance = Math.max(Number.parseFloat(licenceFeeString) -
            Number.parseFloat(transactionAmountTotalString), 0);
        return outstandingBalance;
    };
    const deleteLicenceTransaction = (clickEvent) => {
        clickEvent.preventDefault();
        const transactionIndex = clickEvent.currentTarget.closest('tr').dataset.transactionIndex;
        const doDelete = () => {
    ...
    const openAddTransactionModal = (clickEvent) => {
        ...
        const setTransactionAmountFunction = (clickEvent) => {
            clickEvent.preventDefault();
            const transactionAmountSpanId = clickEvent.currentTarget
                .dataset.spanId;
