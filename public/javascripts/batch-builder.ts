...
;(() => {
  ...
  const transactionBatchesTableElement = document.querySelector(
  ...
  const clearBatch = (clickEvent: Event) => {
    ...
    const batchDateString = (clickEvent.currentTarget as HTMLElement).closest(
      'th'
    )?.dataset.batchDateString

    const doClear = () => {
      cityssm.postJSON(
        urlPrefix + '/batches/doClearBatchTransactions',
        {
          batchDateString
        },
        (responseJSON: {
  ...
  const createOrUpdateBatchTransactionAmount = (changeEvent: Event) => {
    ...
    const batchDateString =
      transactionAmountElement.closest('td').dataset.batchDateString
    const licenceId = transactionAmountElement.closest('tr').dataset.licenceId

    cityssm.postJSON(
      urlPrefix + '/batches/doCreateOrUpdateBatchTransaction',
      {
        licenceId,
        batchDateString,
        transactionAmount
      ...
      (responseJSON: {
        ...
        if (responseJSON.message) {
          const licenceNumber =
            transactionAmountElement.closest('tr').dataset.licenceNumber
  ...
  const renderBatchDateColumns = () => {
    ...
    const tableCellElements = transactionBatchesTableElement.querySelectorAll(
      '[data-batch-date-string]'
    )
    ...
    const tableRowElements = transactionBatchesTableElement.querySelectorAll(
      'tbody tr'
    ...
    for (const batchDateString of batchDateStrings) {
      ...
      thHeadElement.innerHTML =
        ...
        ('<div class="level-right">' +
          ...
          ('<div class="dropdown is-up is-right has-text-weight-normal">' +
            ...
            urlPrefix +
            '/reports/licenceTransactions-byDate?batchDateString=' +
            batchDateString +
            '">' +
        ...
        glm.getDayName(batchDateString) +
        '</div>'

      thHeadElement
        .querySelector('.is-clear-batch-button')
        .addEventListener('click', clearBatch)

      transactionBatchesTableElement
      ...
      transactionBatchesTableElement
        .querySelector('tfoot th:last-child')
      ...
      for (const tableRowElement of tableRowElements) {
        ...
        inputElement.addEventListener(
          'change',
          createOrUpdateBatchTransactionAmount
        )
  ...
  let batchTransactions =
    exports.batchTransactions as recordTypes.LicenceTransaction[]
  ...
  const renderBatchTransactions = () => {
    ...
    const transactionAmountElements =
      transactionBatchesTableElement.querySelectorAll(
        "input[data-field='transactionAmount']"
    ...
    for (const transaction of batchTransactions) {
      ...
      const inputCellElement = transactionBatchesTableElement
        .querySelector(
          "tr[data-licence-id='" + transaction.licenceId.toString() + "']"
        )
        .querySelector(
          "td[data-batch-date-string='" + transaction.batchDateString + "']"
        )
      ...
      inputElement.value = (inputValue + transaction.transactionAmount).toFixed(
        2
      )
      ...
      if (licenceTotalsMap.has(transaction.licenceId)) {
        licenceTotalsMap.set(
          transaction.licenceId,
          licenceTotalsMap.get(transaction.licenceId) +
      ...
      if (batchTotalsMap.has(transaction.batchDateString)) {
        batchTotalsMap.set(
          transaction.batchDateString,
          batchTotalsMap.get(transaction.batchDateString) +
            transaction.transactionAmount
        )
      } else {
        batchTotalsMap.set(
    ...
    const licenceRowElements = transactionBatchesTableElement.querySelectorAll(
      'tbody tr[data-licence-id]'
    ) as NodeListOf<HTMLTableRowElement>
    ...
    for (const licenceRowElement of licenceRowElements) {
      ...
      const outstandingBalance = Number.parseFloat(
        licenceRowElement.dataset.outstandingBalance
      )
    ...
    const batchTotalElements = transactionBatchesTableElement.querySelectorAll(
      'tfoot th[data-batch-date-string]'
  ...
  const splitOutstandingBalance = (clickEvent: Event) => {
    ...
    const licenceTrElement = (clickEvent.currentTarget as HTMLElement).closest(
      'tr'
    )
    ...
    const doSplit = () => {
      cityssm.postJSON(
        urlPrefix + '/batches/doSplitOutstandingBalances',
        {
    ...
    if (batchDateStrings.length === 0) {
      ...
      bulmaJS.confirm({
        ...
        message:
          'Are you sure you want to evenly split the outstanding balance across ' +
          batchDateStrings.length +
          ' batch' +
          (batchDateStrings.length === 1 ? '' : 'es') +
          '?',
        okButton: {
  ...
  const splitOutstandingBalances = (clickEvent: Event) => {
    ...
    const licenceTrElements = transactionBatchesTableElement.querySelectorAll(
      'tbody tr'
    ) as NodeListOf<HTMLTableRowElement>
    ...
    if (batchDateStrings.length === 0) {
      ...
      bulmaJS.confirm({
        ...
        message:
          'Are you sure you want to evenly split the outstanding balances across ' +
          batchDateStrings.length +
          ' batch' +
          (batchDateStrings.length === 1 ? '' : 'es') +
          '?',
  ...
  const splitOutstandingBalanceButtons = document.querySelectorAll(
    '.is-split-outstanding-balance-button'
  )

  for (const splitOutstandingBalanceButton of splitOutstandingBalanceButtons) {
    splitOutstandingBalanceButton.addEventListener(
      'click',
  ...
  document
    .querySelector('.is-split-outstanding-balances-button')
    .addEventListener('click', splitOutstandingBalances)
  ...
  const clearLicenceTransactions = (clickEvent: Event) => {
    ...
    const licenceId = (clickEvent.currentTarget as HTMLElement).closest('tr')
      .dataset.licenceId

    const doClear = () => {
      cityssm.postJSON(
        urlPrefix + '/batches/doClearLicenceBatchTransactions',
        {
    ...
    bulmaJS.confirm({
      ...
      message:
        'Are you sure you want to clear all transaction amounts in this row?',
      contextualColorName: 'warning',
      okButton: {
  ...
  const clearLicenceButtonElements = document.querySelectorAll(
    '.is-clear-licence-button'
  )

  for (const clearLicenceButtonElement of clearLicenceButtonElements) {
    clearLicenceButtonElement.addEventListener(
      'click',
      clearLicenceTransactions
    )
  }

  document
    .querySelector('.is-clear-licences-button')
    .addEventListener('click', clearAllLicenceTransactions)
  ...
  document
    .querySelector('#is-add-batch-button')
    .addEventListener('click', (clickEvent) => {
      ...
      cityssm.openHtmlModal('batchBuilder-addBatch', {
        onshown: (modalElement, closeModalFunction) => {
          addBatch_batchDateStringElement = modalElement.querySelector(
            '#addBatch--batchDateString'
          )
          addBatch_closeModalFunction = closeModalFunction

          modalElement
            .querySelector('#form--addBatch')
            .addEventListener('submit', doAddBatch)
        },
        onremoved: () => {
