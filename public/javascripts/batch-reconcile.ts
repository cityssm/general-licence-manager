...
;(() => {
  ...
  const batchDate = (
    document.querySelector('#batchReconcile--batchDate') as HTMLInputElement
  ).value
  const externalReceiptNumberElement = document.querySelector(
    '#batchReconcile--externalReceiptNumber'
  ) as HTMLInputElement

  const getTransactionDetails = (clickEvent: Event) => {
    const panelBlockElement = (clickEvent.currentTarget as HTMLElement).closest(
      '.panel-block'
    ) as HTMLElement

    return {
      panelBlockElement,
      successButtonElement: panelBlockElement.querySelector(
        '.is-success-transaction-button'
      ) as HTMLButtonElement,
      failButtonElement: panelBlockElement.querySelector(
        '.is-fail-transaction-button'
      ) as HTMLButtonElement,
  ...
  const markTransactionAsSuccessful = (clickEvent: Event) => {
    ...
    const doSuccess = () => {
      cityssm.postJSON(
        urlPrefix + '/batches/doMarkBatchTransactionSuccessful',
        {
        ...
        (responseJSON: { success: boolean }) => {
          ...
          transactionDetails.successButtonElement.classList.remove(
            'is-outlined'
          )
    ...
    bulmaJS.confirm({
      ...
      message:
        'Are you sure you want to mark this transaction as received successfully?',
  ...
  const successButtonElements = document.querySelectorAll(
    '.is-success-transaction-button'
  )
  ...
  const markTransactionAsFailed = (clickEvent: Event) => {
    ...
    const doFailed = () => {
      cityssm.postJSON(
        urlPrefix + '/batches/doMarkBatchTransactionFailed',
        {
          licenceId: transactionDetails.licenceId,
          transactionIndex: transactionDetails.transactionIndex,
          transactionAmount: transactionDetails.transactionAmount,
  ...
  const failButtonElements = document.querySelectorAll(
    '.is-fail-transaction-button'
  )
