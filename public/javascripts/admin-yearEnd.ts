...
;(() => {
  ...
  const showProcessButtonElement = document.querySelector(
    '#yearEnd--showProcess'
  )
  ...
  document
    ...
    .addEventListener('click', (clickEvent) => {
      ...
      const doRefresh = () => {
        cityssm.postJSON(
          urlPrefix + '/admin/doRefreshDatabase',
          {},
          (responseJSON: { success: boolean }) => {
            if (responseJSON.success) {
              bulmaJS.alert({
                ...
                message:
                  'Be sure to verify your data before making significant changes.',
                contextualColorName: 'success'
              })
      ...
      if (refreshRanSuccessfully) {
        bulmaJS.confirm({
          ...
          message:
            'Refreshing again will delete all of the records from the database.  Are you sure you want to refresh again?',
        ...
        bulmaJS.confirm({
          ...
          message:
            'Are you sure you want to proceed without backing up the database first?',
          contextualColorName: 'danger',
          okButton: {
