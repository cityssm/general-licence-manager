...
export default function addRelatedLicence(licenceIdA, licenceIdB, database) {
    ...
    const result = database
        .prepare('insert or ignore into RelatedLicences (licenceIdA, licenceIdB) values (?, ?)')
        .run(Math.min(licenceIdA_number, licenceIdB_number), Math.max(licenceIdA_number, licenceIdB_number));
    if (closeDatabase) {
        database.close();
    }
