...
export function getCategorySlug(
  ...
  while (categorySlug.lastIndexOf('-') > -1) {
    categorySlug = categorySlug.slice(
      0,
      Math.max(0, categorySlug.lastIndexOf('-'))
    )
...
function getCategoryNDigitsLicenceNumber(
  ...
  const licenceNumberIndex =
    Number.parseInt(
      licenceNumber.slice(Math.max(0, licenceNumber.lastIndexOf('-') + 1)),
      10
    ) + 1

  return (
    categorySlug + '-' + licenceNumberIndex.toString().padStart(digits, '0')
  )
}

const getNextYearNDigitsLicenceNumber = (
  database: sqlite.Database,
  digits: number
): string => {
